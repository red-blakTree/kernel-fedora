...
# ---- clang + ThinLTO（与 copr-linux-cachyos 的 kernel-cachyos-lto.spec 同款）------
# _lto_args 必须同时作用于 %prep 的 olddefconfig：Kconfig 的 HAS_LTO_CLANG 是按 $(CC)
# 是否为 clang 判定的，若用 gcc 跑 olddefconfig，LTO_CLANG_THIN 会被静默丢掉
# （与 build-plan 5.2 节的 X86_64_VERSION 是同一类坑）。
%global _build_lto   1
%if %{_build_lto}
...
# LTO 下必须带同一套 clang 参数跑，否则 HAS_LTO_CLANG 判定为 n，LTO_CLANG_THIN 被丢弃。
